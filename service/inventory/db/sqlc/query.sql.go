...
type GetItemsRow struct {
	...
	GroupID       string `db:"group_id"`
	Groups        string `db:"groups"`
	AmountInStock int64  `db:"amount_in_stock"`
	Tag           string `db:"tag"`
	Description   string `db:"description"`
	IsDefault     bool   `db:"is_default"`
	Price         int64  `db:"price"`
...
const getOpeningStock = `-- name: GetOpeningStock :one
...
type GetOpeningStockRow struct {
	ID              string         `db:"id"`
	FormNumber      string         `db:"form_number"`
	TransactionDate time.Time      `db:"transaction_date"`
	WarehouseID     string         `db:"warehouse_id"`
...
func (q *Queries) GetOpeningStock(ctx context.Context, id string) (GetOpeningStockRow, error) {
	row := q.db.QueryRowContext(ctx, getOpeningStock, id)
	var i GetOpeningStockRow
	err := row.Scan(
		&i.ID,
		&i.FormNumber,
		&i.TransactionDate,
...
const getOpeningStocks = `-- name: GetOpeningStocks :many
SELECT a.id, a.form_number, a.transaction_date, a.warehouse_id, a.warehouse_rack_id,
b.item_id, c.name AS item_name, a.variant_id, b.name AS variant_name,
a.item_unit_id, e.name AS item_unit_name, a.item_unit_value,
a.amount, a.price, a.batch, a.expired_date
...
type GetOpeningStocksParams struct {
	StartDate    time.Time `db:"start_date"`
	EndDate      time.Time `db:"end_date"`
	WarehouseIds []string  `db:"warehouse_ids"`
}

type GetOpeningStocksRow struct {
	ID              string         `db:"id"`
	FormNumber      string         `db:"form_number"`
	TransactionDate time.Time      `db:"transaction_date"`
	WarehouseID     string         `db:"warehouse_id"`
...
func (q *Queries) GetOpeningStocks(ctx context.Context, arg GetOpeningStocksParams) ([]GetOpeningStocksRow, error) {
	rows, err := q.db.QueryContext(ctx, getOpeningStocks, arg.StartDate, arg.EndDate, pq.Array(arg.WarehouseIds))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetOpeningStocksRow
	for rows.Next() {
		var i GetOpeningStocksRow
		if err := rows.Scan(
			&i.ID,
			&i.FormNumber,
			&i.TransactionDate,
...
RETURNING id, company_id, branch_id, source_warehouse_id, destination_warehouse_id, form_number, transaction_date, is_received, is_deleted, created_at, updated_at
`
...
const insertOpeningStock = `-- name: InsertOpeningStock :exec
...
type InsertOpeningStockParams struct {
	ID              string         `db:"id"`
	FormNumber      string         `db:"form_number"`
	TransactionDate time.Time      `db:"transaction_date"`
	CompanyID       string         `db:"company_id"`
...
func (q *Queries) InsertOpeningStock(ctx context.Context, arg InsertOpeningStockParams) error {
	_, err := q.db.ExecContext(ctx, insertOpeningStock,
		arg.ID,
		arg.FormNumber,
		arg.TransactionDate,
		arg.CompanyID,
...
const upsertOpeningStock = `-- name: UpsertOpeningStock :exec
INSERT INTO inventory.opening_stocks(id,
form_number, transaction_date, company_id, branch_id, warehouse_id, warehouse_rack_id,
variant_id, item_unit_id, item_unit_value, amount, price,
batch, expired_date, item_barcode_id)
...
type UpsertOpeningStockParams struct {
	ID              string         `db:"id"`
	FormNumber      string         `db:"form_number"`
	TransactionDate time.Time      `db:"transaction_date"`
	CompanyID       string         `db:"company_id"`
...
func (q *Queries) UpsertOpeningStock(ctx context.Context, arg UpsertOpeningStockParams) error {
	_, err := q.db.ExecContext(ctx, upsertOpeningStock,
		arg.ID,
		arg.FormNumber,
		arg.TransactionDate,
		arg.CompanyID,
