...
func (q *Queries) GetPOSPaymentMethod(ctx context.Context, name string) ([]GetPOSPaymentMethodRow, error) {
	rows, err := q.db.QueryContext(ctx, getPOSPaymentMethod, name)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
