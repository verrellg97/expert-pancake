...
import (
	...
	"github.com/expert-pancake/service/business-relation/impl"
...
func (c *component) New() error {
	...
	businessRelationTrx := db.NewBusinessRelationTrx(conn)

	validator := validator.NewValidator()

	businessRelationService := impl.NewBusinessRelationService(config, validator, businessRelationTrx)

	server := &http.Server{
		Addr:    fmt.Sprintf(":%s", config.Server.Port),
		Handler: c.Routes(businessRelationService),
	}
...
func main() {
	log.Println("Starting business relation service")
