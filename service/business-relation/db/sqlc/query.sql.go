...
const getContactInvitations = `-- name: GetContactInvitations :many
...
ON ((a.primary_company_id = e.primary_company_id AND e.secondary_company_id = $1::text)
OR (a.primary_company_id = e.secondary_company_id AND e.primary_company_id = $1::text))
AND (e.status = 'waiting' OR e.status = 'accepted') 
WHERE a.primary_company_id <> $1::text
...
type GetContactInvitationsRow struct {
	ID                  string `db:"id"`
	KonekinID           string `db:"konekin_id"`
	PrimaryCompanyID    string `db:"primary_company_id"`
	Name                string `db:"name"`
...
func (q *Queries) GetContactInvitations(ctx context.Context, companyID string) ([]GetContactInvitationsRow, error) {
	rows, err := q.db.QueryContext(ctx, getContactInvitations, companyID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetContactInvitationsRow
	for rows.Next() {
		var i GetContactInvitationsRow
		if err := rows.Scan(
			&i.ID,
			&i.KonekinID,
			&i.PrimaryCompanyID,
...
type GetRequestInvitationsRow struct {
	ID                  string `db:"id"`
	Status              string `db:"status"`
	ContactBookID       string `db:"contact_book_id"`
	KonekinID           string `db:"konekin_id"`
...
func (q *Queries) GetRequestInvitations(ctx context.Context, primaryCompanyID string) ([]GetRequestInvitationsRow, error) {
	rows, err := q.db.QueryContext(ctx, getRequestInvitations, primaryCompanyID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetRequestInvitationsRow
	for rows.Next() {
		var i GetRequestInvitationsRow
		if err := rows.Scan(
			&i.ID,
			&i.Status,
			&i.ContactBookID,
