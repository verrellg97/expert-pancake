...
func (q *Queries) GetContactInvitations(ctx context.Context, primaryCompanyID string) ([]GetContactInvitationsRow, error) {
	rows, err := q.db.QueryContext(ctx, getContactInvitations, primaryCompanyID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
